...
export function NotionWidget() {
  ...
  // const [aspectRatioLoaded, setAspectRatioLoaded] = useState(false);
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      setError(
        "Failed to connect to Notion. Please check your API configuration."
      );
    } finally {
      setIsLoading(false);
    }
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 bg-[#ebebeb] text-[#606060] rounded-full hover:scale-105 transition-all border border-gray-200 text-sm font-medium"
                >
                  {aspectRatioLoaded ? (aspectRatio === "1:1" ? "4:5" : "1:1") : <Loader className="animate-spin size-4" />}
                  <Expand className="w-4 h-4" />
                </button>
              </TooltipTrigger>
              <TooltipContent side="bottom">Change aspect ratio</TooltipContent>
            </Tooltip>
          </div> */}
        </div>
