...
const getNotionClient = (token: string) => {
...
export async function getMediaPosts(queryParams: {
  databaseId: string;
  token: string;
}): Promise<MediaPost[]> {
  const { databaseId, token } = queryParams;
  console.log("Using Database ID:", databaseId); // Debug log for database ID
